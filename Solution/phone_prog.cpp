...
bool islem_yap(char secim){
	...
	case'S':case's':
		...
		break;
	case'C':case'c':
		cout << "Are you sure taht you want to terminate the program? (E/H):";
		cin >> secim;
		if (secim == 'E' || secim == 'e')
			sonlandir = true;
		break;
...
void kayit_ekle(){
	Tel_Kayit yenikayit;
	cout << "Please enter the information of the person you want so save " << endl;
	cout << "Name: ";
	cin.ignore(1000, '\n');
...
void kayit_sil(){
	...
	if (kisisayisi == 0){
		cout << "Recort can not be found" << endl;
	}
	else{
		if (kisisayisi == 1){
			cout << "Record has been found." << endl;
			cout << "Please enter the index of the record that you want to delete this contact (Press -1 to exit without deletion): ";
		}
		else
		{
			cout << "Please enter the index of the record that you want to delete (Press -1 without deletion): ";
		}
		cin >> secim;
		if (secim == -1) return;
		defter.sil(secim);
...
void kayit_guncelle(){
	...
	if (kisisayisi == 0){
		...
		if (kisisayisi == 1){
			cout << "Record has been found." << endl;
			cout << "Please enter the index of the record that you want to update this contact (Press -1 to exit without updating): ";
		}
		else{
			cout << "Please enter the index of the record that yu want to update (Press -1 to exit without updating): ";
		}
		cin >> secim;
		if (secim == -1) return;
		Tel_Kayit yenikayit;
		...
		cout << "Record has been updated sucesfully" << endl;
	}
	getchar();
}